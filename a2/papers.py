...
class PaperTree(TMTree):
    ...
    === Private Attributes ===
    _author: the author(s) of this paper
    _doi: the URL of this paper
    ...
    if Papertree is a category, doi = '', author = ''
    """
    ...
    def __init__(self, name: str, subtrees: List[TMTree], authors: str = '',
        ...
        if all_papers is True:
            dict = _load_papers_to_dict(by_year)
            subtrees = _build_tree_from_dict(dict)
...
def _create_dict() -> Dict:
    ...
    with open(DATA_FILE, 'r') as csv_file:
        ...
        for line in csv_reader:
            author = line['Author']
            name = line['Title']
            cate = line['Category'].split(": ")
            doi = line['Url']
            size = line['Citations']
            year = line['Year']
            dict1[name] = _helper_create_dict(author, cate, size, doi, year)
        return dict1

def _helper_create_dict(author: str, cate: List[Any], size: int,
                        doi: str, year: int) -> dict:
...
def _load_papers_to_dict(by_year: bool = True) -> Dict:
    ...
    with open(DATA_FILE, 'r') as csv_file:
        ...
        for line in csv_reader:
            ...
            cate = cate.split(': ')
            if by_year is False:
                dict2 = _create_recursive_dict(cate, author, size, name, doi, year)
                _merge_dict(dict1, dict2)
            else:
                dict2 = _create_recursive_dict(cate, author, size, name, doi,
                                               year)
                _merge_dict(dict3[year], dict2)
        if by_year is False:
            return dict1
        else:
            return dict3
...
def _create_recursive_dict(cate: list, author: str, size: int, name: str,
                        doi: str, year: int)  -> Dict:
    if len(cate) == 1:
        return {cate[0]: {name: (author, size, doi, year)}}
    else:
        result = {}
        result[cate[0]] = _create_recursive_dict(cate[1:], author,
                                                 size, name, doi, year)
        return result


def _merge_dict(dict1:Dict, dict2:Dict) -> None:
    if dict1 == {}:
        sel_key = list(dict2.keys())[0]
        dict1[sel_key] = dict2[sel_key]
    elif list(dict2.keys())[0] not in dict1:
        sel_key = list(dict2.keys())[0]
        dict1[sel_key] = dict2[sel_key]
    else:
        sel_key = list(dict2.keys())[0]
...
def _build_tree_from_dict(nested_dict: Dict) -> List[PaperTree]:
    ...
    for k in nested_dict:
        if isinstance(nested_dict[k], tuple):
            resource = nested_dict[k]
            result.append(PaperTree(k, [], resource[0], resource[2],
                                    resource[1], by_year=False))
        else:

            folder = nested_dict[k]
            for m in folder:
                subtree = _build_tree_from_dict(folder[m])
                result.append(PaperTree(m, subtree, by_year=False))
    return result
